...
class RockafellarUryasevLoss:
    ...
    def __call__(self, y, h_out, alpha_out):

        bound_low = bound_function(x, low=True)
        bound_up = bound_function(x, up=True)

        loss_vals = self.loss(h_out, y)

        ru_loss = bound_low * loss_vals + (1- bound_low) * alpha_out + (bound_up - bound_low) * torch.nn.functional.relu(loss_vals - alpha_out)
