...
def get_shift_one_dim_dataloaders(dataset, n_train, seed, p_train, p_test_lo, p_test_hi, n_test_sweep):
    ...
    for p_test in p_tests:
        X_test, y_test = generate_shift_one_dim_dataset(n=10000, p=p_test, seed=seed + 1)
        X_tests.append(X_test)
        y_tests.append(y_test)

    X_train, y_train = generate_shift_one_dim_dataset(n=n_train, p=p_train, seed=seed + 2)
    return (
...
def get_shift_high_dim_dataloaders(dataset, n_train, d, seed, p_train, p_test_lo, p_test_hi, n_test_sweep):
    X_val, y_val, _, beta = generate_shift_high_dim_dataset(n=int(n_train * 0.2), d=d, p=p_train, seed=seed)
    if n_test_sweep == 5:
    ...
    y_tests = []
    for p_test in p_tests:
        X_test, y_test, _, beta1 = generate_shift_high_dim_dataset(n=20000, d=d, p=p_test, seed=seed + 1)
        np.testing.assert_allclose(beta, beta1)
        X_tests.append(X_test)
        y_tests.append(y_test)

    
    X_train, y_train, _, _ = generate_shift_high_dim_dataset(n=n_train, d=d, p=p_train, seed=seed + 2)
...
def get_dataloaders(X_train, y_train, X_val, y_val, X_tests, y_tests, seed):
    ...
    train_loader = DataLoader(train, batch_size=int(X_train.shape[0]*0.25), shuffle=True)
